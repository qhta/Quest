...
/// <summary>
/// Helper methods for SfTreeGrid.
/// </summary>
public static class SfTreeGridHelper
{
  /// <summary>
  ...
  public static void GetTreeGridAsText(SfTreeGrid grid, DataObject data)
  {
    ...
    line.Clear();
    foreach (var row in grid.View.Nodes)
    {
  ...
  /// <param name="grid"></param>
  /// <param name="data"></param>
  public static void GetTreeGridAsHtml(SfTreeGrid grid, DataObject data)
  {
    ...
    foreach (var row in grid.View.Nodes)
    {
      ...
      {
        var cellValue = GetCellValue(column, row);
        var isNumeric = column is TreeGridNumericColumn && column.MappingName == "Value";
  ...
  /// <param name="node">The node object containing the data.</param>
  /// <returns>The formatted cell value as a string.</returns>
  public static string GetCellValue(TreeGridColumn column, object node)
  {
    if (node is TreeNode treeNode)
    {
      ...
      if (column is TreeGridNumericColumn numericColumn && value is IFormattable formattable)
      {
      ...
      if (column.MappingName == "Value" && value is double doubleValue)
      {
        // Default formatting for "Value" column if not a numeric column
